...
`rotl` has a function that gets for you all info from the node containing a taxon. That includes the _node id_.
```{r, eval = TRUE, warning= FALSE, error = TRUE}
canis_node_info <- rotl::tol_node_info(canis$ott_id)
canis_node_info
```
We can use that _node id_ to get a subtree with `tol_subtree()`
```{r, eval = TRUE, results= 'hide', warning= FALSE, error = TRUE}
...
We can use the function `tax_rank()` to extract all children at or belows the species level.
First, get all ranks.
```{r, eval = TRUE, results= 'hide', warning= FALSE, error = TRUE}
...
What if we want to get all taxa of a taxonomic ranks above the species level?
Try to get all amphibian families.

Extract ott ids from tip labels of subtree of all amphibians.
```{r, eval = TRUE, results= 'hide', warning= FALSE, error = TRUE}
amphibia_ott_ids <- datelife::extract_ott_ids(x = amphibia_subtree$tip.label)
```
Get their taxon information. This might take about 10 minutes or so.
```{r, eval = FALSE, results= 'hide', warning= FALSE, error = TRUE}
amphibia_ott_ids_taxon_info <- rotl::taxonomy_taxon_info(amphibia_ott_ids)
```
...
```{r, eval = FALSE, results= 'hide', warning= FALSE, error = TRUE}
amphibia_ranks <- rotl::tax_rank(amphibia_ott_ids_taxon_info)
...
We have implemented a function to extract all taxa of a given rank. It is also sligthly faster.
...
```{r, eval = TRUE, results= 'hide', warning= FALSE, error = TRUE}
amphibia_families_subtree <- rotl::tol_induced_subtree(amphibia_ott_ids_families)
```
...
# Get branch length information for you taxa
...
apes_dr
```
We have a list of matrices with time of lineage divergence data.
